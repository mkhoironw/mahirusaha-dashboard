...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState({
    ...
    totalToko: 0, tokoAktif: 0,
    mrr: 0, partnerAktif: 0, pendaftarBaru: 0, totalKomisiPending: 0,
  })
  ...
  const [partnerTab, setPartnerTab] = useState<'pendaftar' | 'aktif'>('pendaftar')
  const [editingKomisi, setEditingKomisi] = useState<string | null>(null)
  ...
  const loadAllData = async () => {
    ...
    try {
      const [
        ...
        supabase.from('partner_komisi').select('*').order('created_at', { ascending: false }),
      ])

      const c = clientsData || []
      const s = storesData || []
      const l = leadsData || []
      const p = partnersData || []
      const k = komisiData || []
      ...
      setStats({
        ...
        mrr,
        partnerAktif: p.filter(pt => pt.status === 'aktif').length,
        pendaftarBaru: p.filter(pt => pt.status === 'pending').length,
        totalKomisiPending,
      })
    } catch (err) {
      console.error(err)
  ...
  const handleKirimNotifBotAktif = async (clientId: string, nama: string, email: string) => {
    if (!window.confirm(`Kirim email "Bot Aktif" ke ${nama}?`)) return
    setSendingEmail(clientId)
    try {
      const res = await fetch('/api/email/bot-aktif', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ client_id: clientId, nama, email })
      })
      if (res.ok) alert(`✅ Email berhasil dikirim ke ${email}`)
  ...
  const handleUpdateNomorWA = async (storeId: string) => {
    ...
    setSaving(true)
    try {
      await supabase.from('stores').update({ nomor_wa_toko: newNomorWA }).eq('id', storeId)
  ...
  const handleApprovePartner = async (partnerId: string, nama: string, nomorWa: string) => {
    ...
    try {
      ...
      setPartners(prev => prev.map(p => p.id === partnerId ? { ...p, status: 'aktif' } : p))
      setStats(prev => ({ ...prev, partnerAktif: prev.partnerAktif + 1, pendaftarBaru: Math.max(0, prev.pendaftarBaru - 1) }))
      ...
      alert(`✅ ${nama} berhasil diapprove! Notif WA dikirim.`)
    } catch { alert('❌ Gagal approve.') }
    finally { setApprovingPartner(null) }
  }

  const handleTolakPartner = async (partnerId: string, nama: string) => {
    if (!window.confirm(`Tolak pendaftaran ${nama}?`)) return
    await supabase.from('partners').update({ status: 'ditolak', updated_at: new Date().toISOString() }).eq('id', partnerId)
    setPartners(prev => prev.map(p => p.id === partnerId ? { ...p, status: 'ditolak' } : p))
    setStats(prev => ({ ...prev, pendaftarBaru: Math.max(0, prev.pendaftarBaru - 1) }))
  }

  const handleUpdateKomisi = async (partnerId: string) => {
    setSaving(true)
    try {
      await supabase.from('partners').update({ komisi_persen: newKomisiPersen }).eq('id', partnerId)
      setPartners(prev => prev.map(p => p.id === partnerId ? { ...p, komisi_persen: newKomisiPersen } : p))
      setEditingKomisi(null)
      alert(`✅ Komisi diupdate ke ${newKomisiPersen}%`)
  ...
  const getStatusLabel = (s: string) => ({ aktif: 'Aktif', trial: 'Trial', suspend: 'Suspend' }[s] || s)
  const getPaketLabel = (p: string) => ({ starter: 'Starter', pro: 'Pro', bisnis: 'Bisnis', enterprise: 'Enterprise', trial: 'Trial' }[p] || p)
  ...
  const rejectedPartners = partners.filter(p => p.status === 'ditolak')

  const menuItems = [
    { id: 'overview', icon: '📊', label: 'Overview' },
    { id: 'klien', icon: '👥', label: 'Klien' },
    { id: 'partner', icon: '🤝', label: 'Partner', badge: stats.pendaftarBaru },
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: '#070d1a', color: '#fff', minHeight: '100vh', display: 'flex' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-a { transition: opacity 0.2s; cursor: pointer; }
        ...
        <nav style={{ flex: 1, padding: '12px 8px', overflowY: 'auto' }}>
          {menuItems.map(item => (
            <div key={item.id} className="menu-item" onClick={() => setActiveMenu(item.id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', marginBottom: '2px', cursor: 'pointer', background: activeMenu === item.id ? 'rgba(37,211,102,0.1)' : 'transparent', border: activeMenu === item.id ? '1px solid rgba(37,211,102,0.2)' : '1px solid transparent' }}>
              ...
              {item.badge && item.badge > 0 ? <div style={{ background: '#EF4444', color: '#fff', fontSize: '0.65rem', fontWeight: 700, padding: '2px 6px', borderRadius: '100px', minWidth: '18px', textAlign: 'center' }}>{item.badge}</div> : null}
            </div>
          ))}
        </nav>
      ...
      <div style={{ flex: 1, overflowY: 'auto', height: '100vh' }}>
        <div style={{ padding: '16px 28px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(7,13,26,0.8)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <h1 style={{ fontSize: '1rem', fontWeight: 700 }}>{menuItems.find(m => m.id === activeMenu)?.icon} {menuItems.find(m => m.id === activeMenu)?.label}</h1>
            {activeMenu === 'partner' && stats.pendaftarBaru > 0 && (
              <span style={{ background: '#EF4444', color: '#fff', fontSize: '0.7rem', fontWeight: 700, padding: '2px 10px', borderRadius: '100px' }}>{stats.pendaftarBaru} menunggu review</span>
            )}
          </div>
          <button onClick={loadAllData} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)', padding: '7px 14px', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.8rem' }}>🔄 Refresh</button>
        </div>
            ...
            <div className="fadeUp">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '14px', marginBottom: '16px' }}>
                {[
                  ...
                  <div key={s.label} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '18px' }}>
                    <div style={{ fontSize: '1.5rem', marginBottom: '8px' }}>{s.icon}</div>
                    <div style={{ fontSize: '1.5rem', fontWeight: 800, color: s.color, marginBottom: '2px' }}>{s.value}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '14px', marginBottom: '24px' }}>
                {[
                  ...
                  { label: 'Partner Aktif', value: stats.partnerAktif, icon: '🤝', color: '#818cf8' },
              ...
              {stats.pendaftarBaru > 0 && (
                <div style={{ background: 'rgba(129,140,248,0.08)', border: '1px solid rgba(129,140,248,0.2)', borderRadius: '14px', padding: '14px 20px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 700, fontSize: '0.875rem', color: '#818cf8' }}>{stats.pendaftarBaru} pendaftar partner baru menunggu review</div>
                      <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)' }}>Review dan approve di menu Partner</div>
                    </div>
                  </div>
                  <button onClick={() => { setActiveMenu('partner'); setPartnerTab('pendaftar') }} style={{ background: 'linear-gradient(135deg,#818cf8,#6366f1)', color: '#fff', padding: '8px 16px', borderRadius: '8px', border: 'none', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
                    Review Sekarang →
                  </button>
                </div>
              )}
            ...
            <div className="fadeUp">
              <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
                ...
                <select style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', padding: '10px 14px', borderRadius: '10px', fontSize: '0.875rem', fontFamily: 'inherit', outline: 'none' }} value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
                  {['semua','aktif','trial','suspend'].map(s => <option key={s} value={s} style={{ background: '#111827' }}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
                </select>
              </div>
                ...
                {filteredClients.map((c, i) => (
                  <div key={c.id} style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 1fr 1fr 1.5fr', gap: '12px', padding: '14px 16px', borderTop: i > 0 ? '1px solid rgba(255,255,255,0.04)' : 'none', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                      <button className="btn-a" onClick={() => handleSuspendKlien(c.id, c.status)} style={{ fontSize: '0.65rem', padding: '4px 8px', borderRadius: '6px', border: `1px solid ${c.status === 'suspend' ? 'rgba(37,211,102,0.3)' : 'rgba(239,68,68,0.3)'}`, background: 'transparent', color: c.status === 'suspend' ? '#25d366' : '#EF4444', cursor: 'pointer', fontFamily: 'inherit' }}>
                        {c.status === 'suspend' ? 'Aktifkan' : 'Suspend'}
                      </button>
                      <button className="btn-a" onClick={() => handleKirimNotifBotAktif(c.id, c.nama_pemilik, c.email)} disabled={sendingEmail === c.id} style={{ fontSize: '0.65rem', padding: '4px 8px', borderRadius: '6px', border: '1px solid rgba(37,211,102,0.3)', background: 'transparent', color: '#25d366', cursor: 'pointer', fontFamily: 'inherit', opacity: sendingEmail === c.id ? 0.5 : 1 }}>
                        {sendingEmail === c.id ? '⏳' : '📧 Bot Aktif'}
                      </button>
                    </div>
                  </div>
                ))}
                {filteredClients.length === 0 && <div style={{ textAlign: 'center', padding: '40px', color: 'rgba(255,255,255,0.3)', fontSize: '0.875rem' }}>Tidak ada klien ditemukan</div>}
              </div>
            </div>
          )}
          ...
          {activeMenu === 'partner' && (
            <div className="fadeUp">
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
                {[
                  { id: 'pendaftar', label: '📋 Pendaftar', count: pendingPartners.length, showBadge: true },
                  { id: 'aktif', label: '🤝 Partner Aktif', count: activePartners.length, showBadge: false },
                ].map(tab => (
                  <button key={tab.id} onClick={() => setPartnerTab(tab.id as 'pendaftar' | 'aktif')} style={{ padding: '9px 20px', borderRadius: '10px', border: partnerTab === tab.id ? 'none' : '1px solid rgba(255,255,255,0.1)', background: partnerTab === tab.id ? 'linear-gradient(135deg,#818cf8,#6366f1)' : 'transparent', color: '#fff', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    {tab.label} ({tab.count})
                    {tab.showBadge && tab.count > 0 && <span style={{ background: '#EF4444', color: '#fff', fontSize: '0.65rem', fontWeight: 700, padding: '1px 6px', borderRadius: '100px' }}>{tab.count}</span>}
                  </button>
                ))}
              </div>
              ...
              {partnerTab === 'pendaftar' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {pendingPartners.length === 0 && rejectedPartners.length === 0 ? (
                    ...
                    [...pendingPartners, ...rejectedPartners].map(app => (
                      <div key={app.id} style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${app.status === 'pending' ? 'rgba(129,140,248,0.2)' : 'rgba(239,68,68,0.15)'}`, borderRadius: '16px', padding: '20px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
                          <div style={{ flex: 1 }}>
                            ...
                            {app.jaringan_umkm && (
                              <div style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.6)', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', padding: '10px 12px', marginBottom: '6px', lineHeight: 1.6 }}>
                            ...
                            <div style={{ display: 'flex', gap: '16px', fontSize: '0.72rem', color: 'rgba(255,255,255,0.4)', marginTop: '6px' }}>
                ...
                <div>
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', overflow: 'hidden', marginBottom: '24px' }}>
                    ...
                    {activePartners.length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '40px', color: 'rgba(255,255,255,0.3)', fontSize: '0.875rem' }}>
                        Belum ada partner aktif — approve dari tab Pendaftar
                      </div>
                    ) : (
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
                            <div style={{ flex: 1 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
                                ...
                                <span style={{ fontSize: '0.68rem', background: 'rgba(37,211,102,0.15)', color: '#25d366', padding: '2px 8px', borderRadius: '100px', fontWeight: 600 }}>Aktif</span>
                              </div>
                              ...
                              <div style={{ display: 'flex', gap: '16px', fontSize: '0.72rem' }}>
                                <span style={{ color: '#818cf8', fontFamily: 'monospace', fontWeight: 700 }}>Kode: {p.referral_code}</span>
                                <span style={{ color: 'rgba(255,255,255,0.4)' }}>Total komisi: <strong style={{ color: '#25d366' }}>{fmt(komisiList.filter(k => k.partner_id === p.id).reduce((sum, k) => sum + k.jumlah_komisi, 0))}</strong></span>
                                <span style={{ color: 'rgba(255,255,255,0.4)' }}>Pending: <strong style={{ color: '#EF9F27' }}>{fmt(komisiList.filter(k => k.partner_id === p.id && k.status === 'pending').reduce((sum, k) => sum + k.jumlah_komisi, 0))}</strong></span>
                              </div>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                              {editingKomisi === p.id ? (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                  ...
                                  <button onClick={() => handleUpdateKomisi(p.id)} disabled={saving} style={{ padding: '6px 14px', borderRadius: '8px', border: 'none', background: 'linear-gradient(135deg,#25d366,#128c7e)', color: '#fff', fontWeight: 700, fontSize: '0.78rem', cursor: 'pointer', fontFamily: 'inherit' }}>{saving ? '...' : 'Simpan'}</button>
                                  <button onClick={() => setEditingKomisi(null)} style={{ padding: '6px 10px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontFamily: 'inherit' }}>Batal</button>
                                </div>
                              ) : (
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                  ...
                                  <button className="btn-a" onClick={() => { setEditingKomisi(p.id); setNewKomisiPersen(p.komisi_persen || 15) }} style={{ padding: '8px 14px', borderRadius: '8px', border: '1px solid rgba(129,140,248,0.3)', background: 'transparent', color: '#818cf8', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.78rem', fontWeight: 600 }}>✏️ Edit Komisi</button>
                                </div>
                              )}
                            </div>
                          </div>
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', overflow: 'hidden' }}>
                    ...
                    {komisiList.length === 0 ? (
                      ...
                      <>
                        <div style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 1fr 1fr', gap: '12px', padding: '10px 20px', background: 'rgba(255,255,255,0.04)', fontSize: '0.72rem', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                          <span>Partner</span><span>Klien Referral</span><span>Komisi</span><span>Periode</span><span>Status</span>
                        </div>
                        {komisiList.map((k, i) => {
                          const partner = partners.find(p => p.id === k.partner_id)
                          const referred = clients.find(c => c.id === k.referred_client_id)
                          return (
                            <div key={k.id} style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 1fr 1fr', gap: '12px', padding: '14px 20px', borderTop: '1px solid rgba(255,255,255,0.04)', alignItems: 'center' }}>
                              <div>
                                <div style={{ fontWeight: 600, fontSize: '0.82rem' }}>{partner?.nama_lengkap || '-'}</div>
                                <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>{partner?.komisi_persen || 15}% komisi</div>
                              </div>
                              <div>
                                <div style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.6)' }}>{referred?.nama_pemilik || '-'}</div>
                                <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>{referred ? getPaketLabel(referred.paket) : '-'}</div>
                              </div>
                              ...
                              <div>
                                {k.status === 'pending' ? (
                                  <button className="btn-a" onClick={() => handleBayarKomisi(k.id)} style={{ fontSize: '0.68rem', padding: '5px 10px', borderRadius: '6px', border: '1px solid rgba(37,211,102,0.3)', background: 'transparent', color: '#25d366', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>✅ Tandai Dibayar</button>
                                ) : (
                                  <div>
                                    <div style={{ fontSize: '0.72rem', color: '#25d366', fontWeight: 600 }}>✅ Dibayar</div>
                                    {k.tanggal_dibayar && <div style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.3)' }}>{new Date(k.tanggal_dibayar).toLocaleDateString('id-ID')}</div>}
                                  </div>
                                )}
                              </div>
                            </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-end' }}>
                        <select value={lead.status} onChange={e => handleUpdateLeadStatus(lead.id, e.target.value)} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', padding: '7px 12px', borderRadius: '8px', fontSize: '0.78rem', fontFamily: 'inherit', outline: 'none', cursor: 'pointer' }}>
                          {['baru','dihubungi','demo','negosiasi','closing','batal'].map(s => <option key={s} value={s} style={{ background: '#111827' }}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
                        </select>
                        {lead.nomor_wa && (
            ...
            <div className="fadeUp">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '14px', marginBottom: '24px' }}>
                {[
                  ...
                  <div key={s.label} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '20px' }}>
                    <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</div>
